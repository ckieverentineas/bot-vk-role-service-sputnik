...
export async function Counter_PK_Module(context: any) {
    ...
    if (id == null) { return }
    if (context.text == `!пкметр` || context.text == `📐 Пкметр`) { users_pk[id].mode = true; await context.send(`✅ Активирован режим замера ПК. Вводите рп посты, или любой другой текст длины, какая нужна вам! Когда закончите, нажмите !кончить, чтобы обнулить счет, нажмите !обнулить`); return true }
	if (context.text == `!обнулить` || context.text == `${caller} !обнулить`) { users_pk[id].text = ``; await context.send(`🗑️ Обнулен счетчик режима замера ПК`); return true }
    if (context.text == `!кончить` || context.text == `${caller} !кончить`) { users_pk[id].mode = false; users_pk[id].text = ``; await context.send(`⛔ Обнулен и выключен режим замера ПК`); return true }
